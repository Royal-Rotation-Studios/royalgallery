...
SMODS.Joker{
    key = "capitalism",
    config = { extra = { required_rank = 8, cash_reward = 10, passed = false, processed_round = -1 } },
    pos = { x = 6, y = 0 },
    rarity = 3,
    cost = 7,
    ...
    calculate = function(self, card, context)
        ...
        if context.end_of_round and not context.blueprint then
            ...
            --Pick one random joker and make it rental
            local pool = {}
            for _, j in ipairs(G.jokers.cards) do
                if j ~= card and not j.ability.rental then
                    pool[#pool + 1] = j
                end
            end

            if #pool > 0 then
                local unlucky = pseudorandom_element(pool, pseudoseed("capitalism_rental_" .. tostring(current_round)))
                if unlucky then
                    unlucky.ability.rental = true
                    if unlucky.set_cost then unlucky:set_cost() end
                    shakecard(unlucky)
    ...
    loc_vars = function(self, info_queue, card)
        return { vars = { card.ability.extra.required_rank, card.ability.extra.cash_reward }, key = self.key }
    end
}
